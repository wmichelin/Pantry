...
| Recipe reads/search/tags/delete | `household.tsx`, `recipe/[id].tsx`, board duplicate lookup | Go API and live RLS/persistence acceptance verified; independent management flag enabled for staged web cutover | legacy list/detail equality; member/outsider/two-household scope; tag clearing; ingredient/queue cascade and unrelated data preservation | single atomic parent delete; hidden/missing mutations return not-found rather than false success |
| Running queue | `household.tsx`, `recipe/[id].tsx`, `week-queue.tsx` | Go/API SQL and live acceptance verified; independent queue flag enabled for staged web cutover | ordered legacy projection, exact recipe lookup, duplicate add/repeated remove, two-household rejection, injected clear rollback and manual/foreign-household preservation | add is idempotent and rejects cross-household links; clear queue/checks is atomic, preserves manual items |
| Shopping list | `shopping-list.tsx` | Deferred | occurrence aggregation, manual/check identity, ordering and clear-week behavior | shopping clear-week must delete queue/checks/manual items together; distinct from queue clear |
