...
| Recipe import | `app/(app)/review-recipe.tsx` | ImportRecipe persistence deployed and browser-verified in staging (PR 47) | exact legacy/Go metadata and ingredient row comparison, null/zero and empty imports, long text, true outsider, rollback; see remaining execution plan | import-only 256 KiB request budget; parsing/catalog remain client-owned |
| Board import | `app/(app)/review-board.tsx` | Atomic per-recipe Go persistence available; client orchestration retained | saved/failed counts, existing and within-batch duplicate URLs, lookup failure, best-effort catalog | each recipe atomic; board remains partial-success; count only complete saves and skip successful within-batch duplicates |
| Recipe reads/search/tags/delete | `household.tsx`, `recipe/[id].tsx`, board duplicate lookup | Deployed and browser-verified in staging (PR 48) | legacy list/detail equality; member/outsider/two-household scope; tag clearing; ingredient/queue cascade and unrelated data preservation | single atomic parent delete; hidden/missing mutations return not-found rather than false success |
| Running queue | `household.tsx`, `recipe/[id].tsx`, `week-queue.tsx` | Deployed and three-screen browser-verified in staging (PR 49) | ordered legacy projection, exact recipe lookup, duplicate add/repeated remove, two-household rejection, injected clear rollback and manual/foreign-household preservation | add is idempotent and rejects cross-household links; clear queue/checks is atomic, preserves manual items |
| Shopping checks/clear | `shopping-list.tsx` | Go API and desktop/mobile browser verified in staging (PRs 51–52); independent SHOPPING_CHECKS flag | repeated desired state preserves IDs; separate recipe/manual keys; Unicode legacy uncheck; outsider snapshots; injected second/third-delete rollback; browser failed-request recovery and canceled clear | shopping clear atomically removes queue/checks/manuals; standalone legacy check can be durably unchecked without removing a recipe check; web checkbox state explicitly exposed to accessibility |
| Shopping aggregation/manual/order | `shopping-list.tsx` | Deferred | occurrence aggregation, catalog seeding, manual add/remove, ordering | legacy paths remain explicitly in use during scoped check/clear rollout |
