...
| Shopping checks/clear | `shopping-list.tsx` | Go API/live acceptance passed; independent staging web flag, browser gate pending (PR 51) | repeated desired state preserves IDs; separate recipe/manual keys; Unicode legacy uncheck; outsider snapshots; injected second/third-delete rollback | shopping clear atomically removes queue/checks/manuals; standalone legacy check can be durably unchecked without removing a recipe check |
