...
| Recipe create/import | `app/(app)/create-recipe.tsx`, `review-recipe.tsx` | REST and Connect implemented; staging DB gate blocked (`PGRST202`) | apply `20260902021251_staged_recipe_save_operation.sql` to staging; run acceptance with `verify_recipe=true`; then enable the recipe build flag | individual recipe save becomes atomic; until then staging stays on direct Supabase |
