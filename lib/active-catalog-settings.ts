...
import { catalogSettingsAPI, stagingCatalogSettingsAPIOrigin } from "./catalog-settings-api";
...
export async function activeCatalogSettingsAPI() {
  ...
  if (error || !data.session?.access_token) throw new Error("A valid Pantry session is required.");
