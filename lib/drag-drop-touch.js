...
var DragDropTouch = class {
  ...
  listen() {
    ...
    // Capture + non-passive so we see touches before React/RN-web handlers and can
    // preventDefault once a drag is armed (otherwise iOS scrolls instead).
  ...
  _touchstart(e) {
    if (this._shouldHandle(e)) {
      ...
      if (src) {
        // React / RN-web Pressables often preventDefault on mouse events. Still arm
        // the drag — otherwise touch reorder never starts under React trees.
        this._dispatchEvent(e, `mousemove`, e.target);
        this._dispatchEvent(e, `mousedown`, e.target);
        this._dragSource = src;
        this._ptDown = pointFrom(e);
        this._lastTouch = e;
        // Prefer the draggable as the initial target so touchend can complete a drop
        // even if the finger never moves (press-hold then release).
        this._lastTarget = src;
        // Do not cancel the drag when iOS/React "handles" contextmenu — callout CSS
        // on the row is the real mitigation; resetting here kills press-hold drags.
  ...
  _touchmove(e) {
    ...
    if (this._shouldHandleMove(e) || this._shouldHandlePressHoldMove(e)) {
      ...
      // Notify listeners, but NEVER abort because mousemove was preventDefault'd —
      // React does that constantly and it previously blocked dragstart entirely.
      this._dispatchEvent(e, `mousemove`, target);
      this._lastTouch = e;
      if (this._dragSource && !this._img && this._shouldStartDragging(e)) {
        // Pragmatic (and most HTML5 DnD libs) call preventDefault() on dragstart
        // for custom previews — that must NOT cancel the polyfilled drag.
        this._dispatchEvent(this._lastTouch, `dragstart`, this._dragSource);
        this._createImage(e);
        this._dispatchEvent(e, `dragenter`, target);
        this._lastTarget = target || this._dragSource;
      }
      if (this._img && this._dragSource) {
        ...
        this._moveImage(e);
        // Always treat a real element under the finger as a drop zone. React/Pragmatic
        // may not preventDefault on our synthetic dragover the way desktop browsers do.
        this._dispatchEvent(e, `dragover`, target);
        this._isDropZone = !!target;
   ...
   */
  _touchend(e) {
    if (!this._lastTouch || !this._dragSource) {
      this._reset();
      return;
    }
    // touchend has empty touches[]; use changedTouches via _shouldHandleEnd.
    if (this._shouldHandleEnd(e)) {
      // mouseup preventDefault must not skip drop/dragend (same React issue).
      this._dispatchEvent(this._lastTouch, `mouseup`, this._lastTarget || e.target);
      if (!this._img) {
        this._dragSource = null;
      ...
      this._destroyImage();
      if (this._dragSource) {
        const dropTarget = this._lastTarget || e.target;
        if (e.type.indexOf(`cancel`) < 0 && this._isDropZone) {
          this._dispatchEvent(this._lastTouch, `drop`, dropTarget);
        }
   ...
   */
  _shouldHandle(e) {
    // Ignore defaultPrevented: React/RN-web often mark the event before our bubble
    // listener would see it; we listen in capture now, but stay defensive.
    return e && e.touches && e.touches.length < 2;
  }
  /**
   * touchend/touchcancel expose the finger via changedTouches, not touches.
   */
  _shouldHandleEnd(e) {
    return e && e.changedTouches && e.changedTouches.length > 0;
  }
  /**
   * ...docs go here...
   * @param e
